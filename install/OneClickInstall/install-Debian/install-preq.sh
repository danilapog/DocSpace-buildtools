...
if [[ "${DISTRIB_CODENAME}" =~ ^(focal|bullseye)$ ]]; then
	if dpkg -l | grep -q rabbitmq-server && [ ! -f /etc/apt/sources.list.d/rabbitmq.list ]; then
		...
		if [ "${CHOICE_INSTALLATION}" =~ ^[Yy]$ ]]; then
			...
			echo "deb [signed-by=/usr/share/keyrings/erlang.gpg] https://ppa1.rabbitmq.com/rabbitmq/rabbitmq-erlang/deb/${DIST} ${DISTRIB_CODENAME} main" > /etc/apt/sources.list.d/rabbitmq.list
			echo "deb [signed-by=/usr/share/keyrings/rabbitmq.gpg] https://ppa1.rabbitmq.com/rabbitmq/rabbitmq-server/deb/${DIST} ${DISTRIB_CODENAME} main" >> /etc/apt/sources.list.d/rabbitmq.list
		else
			exit 1
		fi
	fi
